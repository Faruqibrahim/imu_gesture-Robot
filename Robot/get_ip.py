...
def main():
    ...
    try:
        f = open("/media/pi/NK_F/robot/ip_req.txt", "a") # Opening a text file on the USB flash drive
    except:
        try:
            f = open("/media/pi/NK_F/robot/ip_req.txt", "x") # Create a text file if it does not exit on the flash drive
        except:
    ...
    f = open("/media/pi/NK_F/robot/ip_req.txt", "r")
    print(f.read())
    f.close()
